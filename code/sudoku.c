...
static int history[3 * INDEX_LIMIT];
...
static int possible[INDEX_LIMIT];
...
static int tmplt[INDEX_LIMIT];      /* Template indices */
...
reapply (void)
{
    ...
    for (j = 0; j < idx_history; ++j) {
        int hj = history[j];
        if (!(hj & IGNORED) && 0 != GET_DIGIT(hj)) {
            int idx = GET_INDEX(hj);
            if (idx < INDEX_LIMIT) {
                int digit = GET_DIGIT(hj);
...
findhints (void)
{
    ...
    if (0 < n) {
        ...
        for (ii = 0, jj = 1; jj < n; ++jj) {
            if (GET_INDEX(possible[ii]) == GET_INDEX(possible[jj])) {
                /* Let the user make mistakes - do not assume the
                 * board is in a consistent state.
                 */
                if (GET_DIGIT(possible[ii]) == GET_DIGIT(possible[jj])) {
                    possible[ii] |= possible[jj];
                }
            } else {
                ii = jj;
            }
...
static
void
write_title(const char * titlePar)
{
    move(TITLE_LINE, LEFT_LEFT);
    wclrtoeol(stdscr);
    if (0 != titlePar) {
        mvaddstr(TITLE_LINE,
                 (int)((LINE_SIZE - strlen(titlePar)) / 2),
                 titlePar);
    }
}
...
fix (void)
{
    ...
    if (0 == reapply()) {
        ...
        reapply();
        render();
        if (idx_history < INDEX_LIMIT && IS_FIXED(INDEX(cury, curx))) {
            move_next();
        } else {
            move_to(curx, cury);
        }
...
classify (void)
{
    ...
    assert(INDEX_LIMIT == idx_history);
...
generate (void)
{
    static int nonzeroDecimalDigits[9];

    int i;

    for (;;) {
        for (i = 0; i < 9; ++i) {
            nonzeroDecimalDigits[i] = i + 1;
        }

        rotate(nonzeroDecimalDigits, 9, 1 + rand() % 8);
        shuffle(nonzeroDecimalDigits, 9);
        select_template();
        ...
        for (i = 0; i < len_tmplt; ++i) {
            fill(tmplt[i], nonzeroDecimalDigits[i % 9]);
        }
        if (0 != solve() || idx_history < INDEX_LIMIT) {
            continue;
        }
        for (i = 0; i < len_tmplt; ++i) {
            board[tmplt[i]] |= FIXED;
        ...
        for (idx_history = i = 0; i < INDEX_LIMIT; ++i) {
            if (IS_FIXED(i)) {
...
{
    int i;
    for (i = 0; i < INDEX_LIMIT && !IS_EMPTY(i); ++i) {
        /* counting the non-empty squares. */
    }
   return (INDEX_LIMIT == i);
}
...
el_process_delete_ch_at_current_position (edit_t *    ep)
{
    ...
    if (ep->m_0 <= ep->ecp && ep->ecp < ep->ecn) {
        ...
        if (ep->ecp < ep->ecn && ep->ecn > ep->m_0) {
            int first_invisible_loc = ep->m_1 + ep->x_mv;
            int is_scroll_from_left = (ep->m_0 < ep->m_1 &&
                                       ep->ecn < first_invisible_loc);
           ...
           pathname_function        get_pathname)
{
    ...
    if (buffer != NULL && temp_str != NULL) {
        ...
        /* Reset screen */
        clear_status();
        for (actualLine = FILE_LINE; actualLine <= LAST_LINE; ++actualLine) {
            /* Clear the filename and below from the sreen. */
            move(actualLine, LEFT_LEFT);
            wclrtoeol(stdscr);
        }
        move_to(curx, cury);
    }
